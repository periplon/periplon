...
async fn test_workflow_input_variables() {
    ...
    let project_name = workflow.inputs.get("project_name").expect("project_name should exist");
    assert_eq!(project_name.param_type, "string");
    assert_eq!(project_name.required, true);
    assert_eq!(project_name.default, Some(json!("TestProject")));

    // Test environment input
    let environment = workflow.inputs.get("environment").expect("environment should exist");
    assert_eq!(environment.param_type, "string");
    assert_eq!(environment.required, false);
    assert_eq!(environment.default, Some(json!("development")));

    // Test max_retries input (number type)
    let max_retries = workflow.inputs.get("max_retries").expect("max_retries should exist");
    assert_eq!(max_retries.param_type, "number");
    assert_eq!(max_retries.default, Some(json!(3)));

    // Test enable_debug input (boolean type)
    let enable_debug = workflow.inputs.get("enable_debug").expect("enable_debug should exist");
...
async fn test_workflow_output_variables() {
    ...
    let final_report = workflow.outputs.get("final_report").expect("final_report should exist");
...
async fn test_agent_input_variables() {
    ...
    let analyzer = workflow.agents.get("analyzer").expect("analyzer should exist");
    assert_eq!(analyzer.inputs.len(), 1);

    let analysis_depth = analyzer.inputs.get("analysis_depth").expect("analysis_depth should exist");
    assert_eq!(analysis_depth.param_type, "string");
    assert_eq!(analysis_depth.default, Some(json!("deep")));

    // Test builder agent inputs
    let builder = workflow.agents.get("builder").expect("builder should exist");
    assert_eq!(builder.inputs.len(), 2);

    let build_target = builder.inputs.get("build_target").expect("build_target should exist");
    assert_eq!(build_target.param_type, "string");
    assert_eq!(build_target.default, Some(json!("release")));

    let opt_level = builder.inputs.get("optimization_level").expect("optimization_level should exist");
...
async fn test_workflow_variable_interpolation_in_descriptions() {
    ...
    let init = workflow.tasks.get("init_project").expect("init_project should exist");
...
async fn test_agent_variable_interpolation() {
    ...
    let analyze = workflow.tasks.get("analyze_codebase").expect("analyze_codebase should exist");
...
async fn test_task_input_variables() {
    ...
    let config_file = init.inputs.get("config_file").expect("config_file should exist");
    assert_eq!(config_file, &json!("${workflow.project_name}/config.yaml"));

    let debug_mode = init.inputs.get("debug_mode").expect("debug_mode should exist");
    ...
    let project_path = analyze.inputs.get("project_path").expect("project_path should exist");
...
async fn test_variable_scope_hierarchy() {
    ...
    let deploy = workflow.tasks.get("deploy_service").expect("deploy_service should exist");
    assert_eq!(deploy.inputs.len(), 4);

    // Workflow scope
    let target_env = deploy.inputs.get("target_env").expect("target_env should exist");
    ...
    // Agent scope
    let build_type = deploy.inputs.get("build_type").expect("build_type should exist");
    assert_eq!(build_type, &json!("${agent.build_target}"));

    // Workflow scope combined with string
    let service_name = deploy.inputs.get("service_name").expect("service_name should exist");
...
async fn test_task_output_variables() {
    ...
    let build = workflow.tasks.get("build_components").expect("build_components should exist");
...
async fn test_variable_types() {
    ...
    let validate = workflow.tasks.get("validate_deployment").expect("validate_deployment should exist");
    assert_eq!(validate.inputs.len(), 4);

    // Number type
    let max_attempts = validate.inputs.get("max_attempts").expect("max_attempts should exist");
    assert_eq!(max_attempts, &json!("${workflow.max_retries}"));

    // Boolean type
    let debug_enabled = validate.inputs.get("debug_enabled").expect("debug_enabled should exist");
    assert_eq!(debug_enabled, &json!("${workflow.enable_debug}"));

    // String types
    let env_name = validate.inputs.get("env_name").expect("env_name should exist");
    assert_eq!(env_name, &json!("${workflow.environment}"));

    let project = validate.inputs.get("project").expect("project should exist");
...
async fn test_variable_interpolation_in_output_paths() {
    ...
    let init = workflow.tasks.get("init_project").expect("init_project should exist");
    assert!(init.output.as_ref().unwrap().contains("${workflow.project_name}"));

    let analyze = workflow.tasks.get("analyze_codebase").expect("analyze_codebase should exist");
    assert!(analyze.output.as_ref().unwrap().contains("${agent.analysis_depth}"));

    let deploy = workflow.tasks.get("deploy_service").expect("deploy_service should exist");
    assert!(deploy.output.as_ref().unwrap().contains("${workflow.project_name}"));

    let generate = workflow.tasks.get("generate_report").expect("generate_report should exist");
    assert!(generate.output.as_ref().unwrap().contains("${workflow.project_name}"));

    let validate = workflow.tasks.get("validate_deployment").expect("validate_deployment should exist");
    assert!(validate.output.as_ref().unwrap().contains("${workflow.environment}"));
...
async fn test_variable_context_api() {
    ...
    // Set agent variables
    ctx.insert(&Scope::Agent("analyzer".into()), "analysis_depth", json!("deep"));
    ctx.insert(&Scope::Agent("builder".into()), "build_target", json!("release"));

    // Set task variables
    ctx.insert(&Scope::Task("init_project".into()), "config_file", json!("config.yaml"));
