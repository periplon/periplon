//! While Loop Demo
//!
//! This example demonstrates While loop patterns in the DSL:
...
async fn main() -> Result<()> {
    ...
    let workflow_path = "examples/workflows/while_loop_demo.yaml";
    ...
    println!("Executing workflow with While loops...\n");
    println!("This will demonstrate:");
    println!("  1. While loop with condition check before each iteration");
    println!("  2. Max iterations safety limit");
    println!("  3. Delays between iterations");
    println!();
    ...
    match executor.execute().await {
        Ok(()) => {
            ...
            if let Some(state) = executor.get_state() {
                ...
                if let Some(results) = state.get_loop_results("count_while_less_than_five") {
                    println!("\nCounting Results: {} iterations", results.len());
                }

                if let Some(results) = state.get_loop_results("monitor_resource") {
                    println!("Monitoring Results: {} checks", results.len());
                }
            }
        }
        Err(e) => {
